...
public sealed class DbExplainTool
{
    ...
    /// environment 可选(缺省用项目 defaultEnvironment)。返回计划行集(columns + rowset TSV,与 db_query 同构)：
    ...
    [Description("查询执行计划(慢查询分析用)。project=项目名(必填)；sql 必填且仅只读语句(写语句返回 SQL_BLOCKED)；environment 可选(缺省用项目 defaultEnvironment)。返回计划行集 columns+rowset TSV(与 db_query 同构)：MySQL 为传统 EXPLAIN 列(id/select_type/table/rows/Extra)，PG 为默认文本格式，SQL Server 为 SHOWPLAN_ALL 计划列(不实际执行)，Oracle 经 DBMS_XPLAN 输出。不支持 EXPLAIN ANALYZE(会实际执行语句)。")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (guardResult.Kind == StatementKind.Write)
        {
            ...
            return QueryResult.Fail(project, db.Type.ToString(), reason, "SQL_BLOCKED", environment: env).ToJson();
        }
        ...
        // 5. 审计 + 返回（TSV 缺省，db_explain 不开放 format 参数，与 db_schema 口径一致）
