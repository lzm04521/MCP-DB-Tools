[CmdletBinding()]
# 基础部署脚本（发布版本）：用于已构建好的发布包（zip 解压后含 McpDbTools.Server.exe）。
# 完成：确认 → 交互询问 → 提权 → 停服 → 迁移数据 → 替换文件 → 安装自启动 → 注册 MCP。
# 不编译代码。源码部署请用 build and install.ps1（它编译后委托本脚本）。
# 注意：本脚本的 param 默认值、Confirm-InstallDir 文案须与 build and install.ps1 保持一致。
param(
    ...
    # 源文件目录（含 McpDbTools.Server.exe 与 wwwroot 等）；留空则用本脚本所在目录（发布 zip 解压目录）。
    [string]$SourceDir = "",
    [switch]$PauseOnExit,
    # -Confirmed：调用方已完成部署计划确认（跳过 Confirm-InstallDir）；端口/承载交互仍按需进行。
    [switch]$Confirmed,
...
function Restart-AsAdministrator {
    ...
    # 源目录透传：build and install.ps1 委托时 SourceDir 指向临时发布目录，提权后须继续可见。
    if ($SourceDir) {
        $commandParts += @("-SourceDir", (ConvertTo-PowerShellLiteral $SourceDir))
    }
...
# 部署计划确认门。本函数在 build and install.ps1 中有一份相同文案副本，修改需同步。
...
# 提权前完成所有交互式询问，返回 hashtable 供提权命令拼装。
# 已通过 -AdminPortParam / -UseScheduledTaskParam 传入的值不再询问（支持 build and install.ps1 委托与提权后重入）。
function Read-InteractiveChoices {
    $choices = @{}

    # 1. nssm 探测 + 计划任务回退（只决定是否用计划任务；nssm 存在与否由管理员进程再次探测）
    $nssmCommand = Get-Command nssm -ErrorAction SilentlyContinue
    if ($null -eq $nssmCommand) {
        if ($UseScheduledTaskParam -eq "ask") {
            $useTask = Read-UseScheduledTaskFallback
            $choices.UseScheduledTask = if ($useTask) { "yes" } else { "no" }
        }
        else {
            $choices.UseScheduledTask = $UseScheduledTaskParam
        }
    }
    else {
    ...
    if ($AdminPortParam -gt 0) {
        $choices.AdminPort = $AdminPortParam
    }
    else {
...
# 源程序预检：发布版本必须有 McpDbTools.Server.exe。放在 UAC 之前，避免无 exe 还弹 UAC。
# 仓库根误运行本脚本时（无 exe）给出明确提示。
if (-not (Test-Path $sourceExePath)) {
    throw "未找到服务程序: $sourceExePath。install.ps1 仅用于发布版本部署；源码部署请用 build and install.ps1。"
}
...
Write-Host "安装目录: $installDirFull"
...
# 全量替换安装目录：用户数据已分离到独立数据目录，可无条件清空安装目录后从源目录复制。
...
# 复制源目录内容（除 install.ps1 自身）到安装目录。
# 不复制 install.ps1：它是部署脚本，不属于运行时程序；升级时由发布包重新带入即可。
Get-ChildItem -Path $sourceDir -Force -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne "install.ps1" } | ForEach-Object {
    Copy-Item -Path $_.FullName -Destination $installDirFull -Recurse -Force
}

if (-not (Test-Path $exePath)) {
    throw "复制后未找到服务程序: $exePath"
}
...
Write-Host "如需调整安装目录、MCP 作用域或服务名，可重新运行部署脚本并传参：-InstallDir D:\Tools\McpDbTools -McpScope local -AdminServiceName McpDbTools.Admin"
}
finally {
    Show-ExitPause
}
